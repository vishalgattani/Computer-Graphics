...
int main( void )
{
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(obj1.incircles), &obj1.incircles[0], GL_STATIC_DRAW);
    glEnableVertexAttribArray(0);
    glVertexAttribPointer(0, 4, GL_FLOAT, GL_FALSE, sizeof(obj1.incircles), 0);
    glBindVertexArray(0);
	...
	do{
		...
		//ModelMatrix = ctrl.getTranslateMatrix()*ctrl.getRotationMatrix()*ctrl.getScaleMatrix()*ModelMatrix;//*modelMatrices[i];	
        ...
        //glDrawArrays(GL_TRIANGLES, 0, obj1.incircles.size() );
        //glBindVertexArray(0);
		...
		for (unsigned int i = 0; i < amount; i++){
			ModelMatrix = ctrl.getTranslateMatrix()*ctrl.getRotationMatrix()*ctrl.getScaleMatrix()*modelMatrices[i];	
			glm::mat4 MVP = ProjectionMatrix * ViewMatrix * ModelMatrix;
		...
		//shader.use();
		//glBindVertexArray(VAO);
		//glDrawArrays(GL_POINTS, 0, obj1.incircles.size());
