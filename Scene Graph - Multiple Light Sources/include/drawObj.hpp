...
void drawObject::calcMatrices(Controller* ctrl,OBJLoader m,const char* vert_filename,const char* frag_filename){
    ...
    glm::vec3 lightposition(object_position.x+5,object_position.y+3,object_position.z+5);
    glBindBuffer(GL_ARRAY_BUFFER, light_buffer);
    glBufferData(GL_ARRAY_BUFFER, 1 * sizeof(glm::vec3), &lightposition[0], GL_STATIC_DRAW);
...
mat4 drawObject::update(int motion, float scaling,float radius, float baseFrequency,vec3 diff){
    ...
    if(motionID==1){
        ...
        mat4 c_translate = translate ( cached_translate, vec3(0,0,0));
        mat4 c_rotate = rotate ( mat4(1.0), 0.1f*previous, vec3(0,1,0) );
        mat4 c_scale = scale (mat4(1.0), vec3(5,5,5));
        thisMatrix = c_rotate*c_scale;
    }
